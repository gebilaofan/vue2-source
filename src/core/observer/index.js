...
export class Observer {
  ...
  constructor(value: any) {
    ...
    // 数组对象单独进行处理  会调用 observeArray 方法  并没有对数组的每一项进行 Object.defineProperty 的定义，也就是说数组内的项是没有 dep 的  目的减少性能损耗   Vue 内部重写了数组的方法  调用这些方法时，数组会更新检测，触发视图更新
    if (Array.isArray(value)) {
      if (hasProto) {
        // 内部重写数组 push pop shift sort等方法
...
export function set(target: Array<any> | Object, key: any, val: any): any {
  ...
  if (key in target && !(key in Object.prototype)) {
    // key 存在target 中 直接赋值返回   因为这样的变化是可以观测到了  数据变动
    target[key] = val
    return val
  }
  ...
  // 如果 ob 不存在 说明这个数据不是一个响应式对象  直接赋值返回
  if (!ob) {
    target[key] = val
    return val
  }

  // 把新添加的属性变成响应式对象  手动触发更新
